    ...
    const setBadgeIfSupported = async (count) => {
        if ('setAppBadge' in self.registration) {
            try {
                ...
                // Ignore unsupported or denied badge updates.
            }
        }
    }

    const clearBadgeIfSupported = async () => {
    ...
    self.addEventListener('activate', event => {
      event.waitUntil(self.clients.claim())
        ...
        const refreshCoreCache = async () => {
            ...
            await Promise.all(
                PERIODIC_SYNC_URLS.map(async (url) => {
                    try {
                        ...
                        // Skip failed requests; we'll try again on next sync tick.
                    }
                })
            )
        }
        ...
        self.addEventListener('message', event => {
            ...
            if (data.type === 'CLEAR_BADGE') {
                event.waitUntil(clearBadgeIfSupported())
            }
        })
