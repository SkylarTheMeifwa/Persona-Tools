// Global Dropbox session expiration modal logic
function setDropboxSessionExpired() {
  try {
    window.localStorage.setItem("dropboxSessionExpired", "1");
  } catch (e) {}
}

function showDropboxSessionExpiredModal() {
  if (document.getElementById("dropboxSessionExpiredModal")) return;
  const modal = document.createElement("div");
  modal.id = "dropboxSessionExpiredModal";
  modal.style = `
  ...
  modal.innerHTML = `
    <div style="background: #fff; padding: 32px 24px; border-radius: 10px; min-width: 320px; max-width: 90vw; box-shadow: 0 2px 24px #0004; text-align: center;">
      <h2 style="margin-bottom: 16px; color: #c00;">Dropbox Session Expired</h2>
      <p style="margin-bottom: 24px;">Your Dropbox session has expired. Please log in again to continue using Dropbox features.</p>
      <button id="dropboxSessionExpiredLoginBtn" style="padding: 10px 24px; font-size: 1.1em; background: #007ee5; color: #fff; border: none; border-radius: 5px; cursor: pointer;">Log In to Dropbox</button>
    </div>
  `;
  // Block all interaction
  document.body.style.overflow = "hidden";
  document.body.appendChild(modal);
  document.getElementById("dropboxSessionExpiredLoginBtn").onclick = function() {
    // Remove modal and unblock scroll
    document.body.style.overflow = "";
    modal.remove();
    window.localStorage.removeItem("dropboxSessionExpired");
    window.location.href = "/api/dropbox";
  };
}

// On every page load, check for Dropbox session expiration flag
if (window.localStorage.getItem("dropboxSessionExpired") === "1") {
  showDropboxSessionExpiredModal();
  // Only show once
  window.localStorage.removeItem("dropboxSessionExpired");
}
